...
class PRReviewRecord(Base):
    ...
    review_id = Column(String(36), unique=True, index=True, default=lambda: str(uuid.uuid4()))
    ...
    # 关联关系
    file_reviews = relationship("FileReviewRecord", back_populates="pr_review", cascade="all, delete-orphan")
    standard_checks = relationship("StandardCheckRecord", back_populates="pr_review", cascade="all, delete-orphan")
...
class FileReviewRecord(Base):
    ...
    issues = relationship("IssueRecord", back_populates="file_review", cascade="all, delete-orphan")
...
class DatabaseManager:
    ...
    def update_pr_review_scores(
        ...
        db = get_db()
        try:
            pr_review = db.query(PRReviewRecord).filter(PRReviewRecord.id == pr_review_id).first()
            if pr_review:
    ...
    @staticmethod
    def get_pr_review_history(repo_full_name: str, limit: int = 50) -> List[PRReviewRecord]:
        """获取PR审查历史"""
        db = get_db()
        try:
            return db.query(PRReviewRecord) \

...
CODE_EXTENSIONS = {
    ...
    '.go', '.rs', '.php', '.rb', '.swift', '.kt', '.scala', '.cs', '.vb',
    '.sql', '.yaml', '.yml', '.xml', '.html', '.css', '.scss', '.less',
    '.sh', '.bash', '.ps1', '.bat', '.dockerfile', '.makefile',
    '.md', '.txt', '.cfg', '.conf', '.ini', '.toml', '.properties',
    '.gradle', '.maven', '.sbt', '.cmake', '.r', '.m', '.pl', '.lua'
}
...
def _is_code_file(filename: str) -> bool:
    ...
    code_filenames = {
        ...
        'requirements.txt', 'package-lock.json', 'yarn.lock', 'composer.lock',
        '.gitignore', '.gitattributes', '.dockerignore', '.eslintrc', '.prettierrc',
        '.babelrc', '.editorconfig', '.env', '.env.example', '.env.local',
        'license', 'changelog', 'contributing', 'authors', 'maintainers'
    }
...
def fetch_pr_and_code_files(state: AgentState) -> AgentState:
    ...
    # 获取 PR 详细信息
    pr_details = provider.get_pr_info(validated_pr_info['repo'], validated_pr_info['number'])

    # 验证和合并PR详细信息
    merged_pr_info = data_validator.validate_pr_info({**validated_pr_info, **pr_details})

    # 获取 PR 修改的文件列表
    all_files = provider.get_pr_files(validated_pr_info['repo'], validated_pr_info['number'])
    ...
    logger.info(f"总文件数: {len(all_files)}, 验证后文件数: {len(validated_files)}, 代码文件数: {len(code_files)}")
...
def generate_summary(state: AgentState) -> AgentState:
    ...
    if enhanced_analysis:
        return {
            **state,
            "overall_summary": enhanced_analysis["summary"]
        }
    ...
    if enhanced_analysis:
        ...
        if enhanced_analysis.get('file_results'):
            ...
            for fa in enhanced_analysis['file_results']:
                ...
                comment_parts.append(f"- **代码质量**: {fa.get('code_quality_score', 80):.1f}/100")
                comment_parts.append(f"- **安全评分**: {fa.get('security_score', 80):.1f}/100")
                comment_parts.append(f"- **问题数量**: {len(fa.get('issues', []))}")
                comment_parts.append(f"- **业务影响**: {fa.get('summary', fa.get('business_impact', '无特殊影响'))}")
                comment_parts.append("")
    ...
    pr_review = PRReview(
        ...
        file_reviews=[
            FileReview(
                filename=review["filename"],
                summary=review["summary"],
                issues=[_safe_create_code_issue(issue) for issue in review.get("issues", [])]
            )
            for review in file_reviews
        ]
    )
...
def _build_single_file_review_prompt(file: Dict[str, Any], pr_info: Dict[str, Any]) -> str:
    """构建单文件审查提示"""
    filename = file.get('filename', 'unknown')
...
def _parse_single_file_response(response: str, file: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        for pattern in json_patterns:
            ...
            if json_match:
                json_str = json_match.group(1) if pattern != r'\{.*\}' else json_match.group(0)
                break
        ...
        for issue in issues:
            if isinstance(issue, dict):
                ...
                validated_issue = {
                    ...
                    'severity': issue.get('severity', issue.get('type', 'info')),  # 确保有 severity
        ...
        file_review = {
            ...
            'best_practices_score': _safe_get_score(result.get('best_practices_score', 80)),
...
def _calculate_overall_scores(file_reviews: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    avg_code_quality = sum(_safe_get_score(fr.get('code_quality_score', 80)) for fr in file_reviews) / total_files
    avg_security = sum(_safe_get_score(fr.get('security_score', 80)) for fr in file_reviews) / total_files
    avg_business = sum(_safe_get_score(fr.get('business_score', 80)) for fr in file_reviews) / total_files
    avg_performance = sum(_safe_get_score(fr.get('performance_score', 80)) for fr in file_reviews) / total_files
    avg_best_practices = sum(_safe_get_score(fr.get('best_practices_score', 80)) for fr in file_reviews) / total_files
    ...
    high_score_files = sum(1 for fr in file_reviews if _safe_get_score(fr.get('score', 80)) >= 85)
...
def _build_comprehensive_review_prompt(pr_info: Dict[str, Any], files: List[Dict[str, Any]]) -> str:
    """构建综合审查提示"""
